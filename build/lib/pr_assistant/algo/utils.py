...
def convert_to_markdown_v2(
    output_data: dict, gfm_supported: bool = True, incremental_review=None, git_provider=None
) -> str:
    """
                        ...
                        if gfm_supported:
                            if get_settings().pr_reviewer.extra_issue_links:
                                ...
                                for component in referenced_variables_list:
                                    ...
                                    if ind != -1:
                                        reference_link_component = git_provider.get_line_link(
                                            relevant_file, component["relevant_line"], component["relevant_line"]
                                        )
                                        issue_content_linked = (
                                            issue_content_linked[: ind - 1]
                                            + f"[`{name}`]({reference_link_component})"
                                            + issue_content_linked[ind + len(name) + 1 :]
                                        )
                                    else:
                                        get_logger().info(
...
def get_max_tokens(model):
    ...
    if model in MAX_TOKENS:
        ...
        raise Exception(
            f"Ensure {model} is defined in MAX_TOKENS in ./pr_assistant/algo/__init__.py or set a positive value for it in config.custom_model_max_tokens"
        )
