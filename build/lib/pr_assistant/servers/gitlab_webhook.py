...
async def get_mr_url_from_commit_sha(commit_sha, gitlab_token, project_id):
    try:
        ...
        response = requests.get(
            f"{gitlab_url}/api/v4/projects/{project_id}/repository/commits/{commit_sha}/merge_requests", headers=headers
        )
...
async def _perform_commands_gitlab(commands_conf: str, agent: PRAssistant, api_url: str, log_context: dict):
