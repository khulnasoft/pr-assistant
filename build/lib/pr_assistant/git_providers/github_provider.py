...
from ..algo.utils import PRReviewHeader, Range, clip_tokens, find_line_number_of_relevant_line_in_file, load_large_diff
...
class GithubProvider(GitProvider):
    ...
    def remove_reaction(self, issue_comment_id: int, reaction_id: str) -> bool:
        try:
            ...
            headers, data_patch = self.pr._requester.requestJsonAndCheck(
                "DELETE", f"{self.base_url}/repos/{self.repo}/issues/comments/{issue_comment_id}/reactions/{reaction_id}"
            )
            return True
        except Exception as e:
        ...
        if deployment_type == "user":
            try:
                ...
                raise ValueError(
                    ...
                    "https://github.com/Khulnasoft/pr-assistant#method-2-run-from-source"
                ) from e
    ...
    def get_lines_link_original_file(self, filepath: str, component_range: Range) -> str:
        ...
        link = f"https://github.com/{self.repo}/blob/{self.last_commit_id.sha}/{filepath}/" f"#L{line_start}-L{line_end}"
        return link
